...
class XMUOJAPI:
    ...
    @staticmethod
    async def make_request(url: str, max_retry: int = 5):
        async with aiohttp.ClientSession() as session:
            for _ in range(max_retry):
                async with session.get(url, headers=XMUOJAPI.headers) as res:
                    if res.status == 200:
                        return await res.read()
        return None

    @staticmethod
    async def make_request_json(url: str, max_retry: int = 5):
        async with aiohttp.ClientSession() as session:
            for _ in range(max_retry):
                async with session.get(url, headers=XMUOJAPI.headers) as res:
                    if res.status == 200:
    ...
    async def get_problem_count():
        info = await XMUOJAPI.make_request_json(
            base_url + "/api/problem?paging=true&offset=0&limit=10000000&page=0"
        )
        return info["total"]

    @staticmethod
    async def get_problem_info(id: str):
        return await XMUOJAPI.make_request_json(
            base_url + f"/api/problem?problem_id={id}"
        )

    @staticmethod
    async def get_problem_testcases(real_id: str):
        return await XMUOJAPI.make_request(
            base_url + f"/api/dl_test_case?problem_id={real_id}"
        )

    @staticmethod
    async def get_problem_list(print_progress: bool = True) -> list[str]:
        total = await XMUOJAPI.get_problem_count()
        per_page = 100
        ...
        with Progress(
            ...
            for page in range(0, max_times + 1):
                page_info = await XMUOJAPI.make_request_json(
                    base_url
...
class XMUOJProblemInfo:
    ...
    @classmethod
    async def create(cls, name, problem_id):
        return cls(name, problem_id)

    def save_problem_json(self, filepath: str):
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(self.info, f, ensure_ascii=False, indent=4)

    async def save_problem_testcases(self, filepath: str):
        #print(f"Downloading testcases for problem {self.name}")
